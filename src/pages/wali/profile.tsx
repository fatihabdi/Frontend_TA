  ...
  React.useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/parent/profile`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const { username, name, email } = response.data.data;
        setParentData({ username, name, email });
      } catch (error) {
      ...
      <Box bg="Base-white" p={5} rounded="md" shadow="lg">
        ...
        <Flex align="center" gap={5} p={7}>
          <Avatar
            ...
            src={`https://ui-avatars.com/api/?name=${parentData.name}`}
            showBorder={true}
            shadow="lg"
          />
          <Box>
        ...
        <Box p={3}>
          <FormControl mt={4}>
            ...
            <Input value={parentData.username} isReadOnly />
          </FormControl>
          <FormControl mt={4}>
            <FormLabel>Email</FormLabel>
            <Input value={parentData.email} isReadOnly />
          </FormControl>
          <FormControl mt={4}>
