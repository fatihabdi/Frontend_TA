...
export default function Pelanggaran() {
  ...
  React.useEffect(() => {
    const fetchViolations = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/student/violation`, {
  ...
  const handleDetailsClick = async (violationId) => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/student/violation/${violationId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
