  ...
  React.useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const { username, name, email, nisn, gender } = response.data.data;
        setStudentData({ username, name, email, nisn, gender });
      } catch (error) {
      ...
      <Box bg="Base-white" p={5} rounded="md" shadow="lg">
        ...
        <Box p={3}>
          <FormControl mt={4}>
            ...
            <Input value={studentData.username} isReadOnly />
          </FormControl>
          <FormControl mt={4}>
            <FormLabel>Email</FormLabel>
            <Input value={studentData.email} isReadOnly />
          </FormControl>
          <FormControl mt={4}>
